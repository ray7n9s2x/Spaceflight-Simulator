...
public class CustomAudienceManagerImplCommon extends CustomAudienceManager {
    ...
    public final android.adservices.customaudience.JoinCustomAudienceRequest convertJoinRequest(JoinCustomAudienceRequest request) {
        android.adservices.customaudience.JoinCustomAudienceRequest joinCustomAudienceRequestBuild = AdFilters$$ExternalSyntheticApiModelOutline0.m448m().setCustomAudience(convertCustomAudience(request.getCustomAudience())).build();
    ...
    public final android.adservices.customaudience.LeaveCustomAudienceRequest convertLeaveRequest(LeaveCustomAudienceRequest request) {
        android.adservices.customaudience.LeaveCustomAudienceRequest leaveCustomAudienceRequestBuild = AdFilters$$ExternalSyntheticApiModelOutline0.m449m().setBuyer(request.getBuyer().convertToAdServices$ads_adservices_release()).setName(request.getName()).build();
    ...
    private final android.adservices.customaudience.CustomAudience convertCustomAudience(CustomAudience request) {
        CustomAudience.Builder trustedBiddingData = AdFilters$$ExternalSyntheticApiModelOutline0.m445m().setActivationTime(request.getActivationTime()).setAds(convertAds(request.getAds())).setBiddingLogicUri(request.getBiddingLogicUri()).setBuyer(request.getBuyer().convertToAdServices$ads_adservices_release()).setDailyUpdateUri(request.getDailyUpdateUri()).setExpirationTime(request.getExpirationTime()).setName(request.getName()).setTrustedBiddingData(convertTrustedSignals(request.getTrustedBiddingSignals()));
    ...
    private final android.adservices.customaudience.TrustedBiddingData convertTrustedSignals(TrustedBiddingData input) {
        ...
        return AdFilters$$ExternalSyntheticApiModelOutline0.m450m().setTrustedBiddingKeys(input.getTrustedBiddingKeys()).setTrustedBiddingUri(input.getTrustedBiddingUri()).build();
    }

...
public final class LimitedDispatcher extends CoroutineDispatcher implements Delay {
    ...
    public void mo12976scheduleResumeAfterDelay(long timeMillis, CancellableContinuation<? super Unit> continuation) {
        this.$$delegate_0.mo12976scheduleResumeAfterDelay(timeMillis, continuation);
    }
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (i >= 16 && LimitedDispatcher.this.dispatcher.isDispatchNeeded(LimitedDispatcher.this)) {
                    LimitedDispatcher.this.dispatcher.mo12975dispatch(LimitedDispatcher.this, this);
                    return;
                }
            }
        }
    ...
    public void mo12975dispatch(CoroutineContext context, Runnable block) {
        Runnable runnableObtainTaskOrDeallocateWorker;
        this.queue.addLast(block);
        if (runningWorkers$volatile$FU.get(this) >= this.parallelism || !tryAllocateWorker() || (runnableObtainTaskOrDeallocateWorker = obtainTaskOrDeallocateWorker()) == null) {
            return;
        }
        this.dispatcher.mo12975dispatch(this, new Worker(runnableObtainTaskOrDeallocateWorker));
    }

...
public final class LongSaturatedMathKt {
    ...
    public static final long m12859saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM12776toLongimpl = Duration.m12776toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m12858checkInfiniteSumDefinedPjuGub4(j, j2, jM12776toLongimpl);
        }
        if ((1 | (jM12776toLongimpl - 1)) == Long.MAX_VALUE) {
            return m12860saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM12776toLongimpl;
        return ((j ^ j3) & (jM12776toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m12858checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m12762isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m12860saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM12733divUwyO8pc = Duration.m12733divUwyO8pc(j2, 2);
        long jM12776toLongimpl = Duration.m12776toLongimpl(jM12733divUwyO8pc, durationUnit);
        return (1 | (jM12776toLongimpl - 1)) == Long.MAX_VALUE ? jM12776toLongimpl : m12859saturatingAddNuflL3o(m12859saturatingAddNuflL3o(j, durationUnit, jM12733divUwyO8pc), durationUnit, Duration.m12765minusLRDsOJo(j2, jM12733divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m12832getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m12831getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m12783unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m12766plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m12783unaryMinusUwyO8pc(infinityOfSign(j3));
        }
